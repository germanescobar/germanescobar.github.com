...
At this point you are ready to start hacking. Notice that the name of the branch has the id of the issue we are working on and a brief description to easily identify it.
...
Before pushing your branch run the following commands:
...
To push a branch to the remote repository run the following command:

<pre class="sunlight-highlight-bash">git push -u origin <name_of_branch></pre>

Now we need to ask the integrator to merge our branch. We do this by opening a pull request[2], which can be done from "Github's":http://www.elibom.com/ web interface. The problem is that "Github":http://www.elibom.com/ creates a new issue for each pull request, but we are already using "Github Issues":https://github.com/blog/831-issues-2-0-the-next-generation. So, what we really want is to attach our pull request to an existing issue.
...
Our pull request is now ready to be merged by the integrator. Notice that the integrator can do some code review at this point, further discuss the issue and even ask for some changes to the code[3].

h2. Caveats

There are some caveats that you should be aware of:

- If there are multiple pull requests opened that conflict with each other, the integrator will have to solve them or merge one and ask the other developer to solve them[4].
...
fn2. Git comes with a pull-request command but it's different from the one of Github.

fn3. Both the developer and the integrator can push more commits to the branch which will become part of the pull request.

fn4. He/she will have to delete the remote branch, rebase and then push again.